        ...
        <form action="{{ route('adminHotelEditConfirm', $hotel->hotel_id) }}" method="post" enctype="multipart/form-data" class="hotel-form">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="form-group">
                    ...
                    <input type="text" id="hotel_name" name="hotel_name" value="{{ old('hotel_name', $hotel->hotel_name) }}" placeholder="ホテル名" class="input input-bordered w-full">
                    @error('hotel_name')
                ...
                <div class="form-group">
                    ...
                    <select id="prefecture_id" name="prefecture_id" class="select select-bordered w-full">
                        ...
                        @foreach($prefectureList as $entry)
                            <option value="{{ $entry['prefecture_id'] }}" {{ old('prefecture_id', $hotel->prefecture_id) == $entry['prefecture_id'] ? 'selected' : '' }}>{{ $entry['prefecture_name'] }}</option>
                        @endforeach
                    </select>
                    @error('prefecture_id')
                    <span class="error text-red-500 text-sm">{{ $message }}</span>
